...
app.get("/api/teams/:teamid/members/:memberid", function(req, res) {
  ...
  console.log(
    "Received a GET request for member " + memberId + " on team " + teamId
  );
...
app.post("/api/teams/:id/members", urlencodedParser, function(req, res) {
  ...
  if (match.TeamGender != "Any" && member.Gender != match.TeamGender) {
    res
      .status(409)
      .send("Member's gender does not conform to team gender rules");
    return;
  }
...
app.put("/api/teams/:id/members", urlencodedParser, function(req, res) {
  ...
  if (match.Age < team.MinMemberAge || match.Age > team.MaxMemberAge) {
    res
      .status(409)
      .send("Member's new age is outside of bounds of team age rules");
    return;
  }
  ...
  if (team.TeamGender != "Any" && match.Gender != team.TeamGender) {
    res
      .status(409)
      .send("Member's new gender does not conform to team gender rules");
    return;
  }
...
app.delete("/api/teams/:teamid/members/:memberid", urlencodedParser, function(
  req,
  res
) {
  let teamId = req.params.teamid;
  let memberId = req.params.memberid;
  console.log(
    "Received a DELETE request for member " + memberId + " on team " + teamId
  );
